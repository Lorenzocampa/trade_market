...
namespace ImGui
{
	class FileBrowser
	{
	  public:
		...
		const std::filesystem::path& GetPwd() const noexcept { return GetDirectory(); }
...
inline ImGui::FileBrowser::FileBrowser(ImGuiFileBrowserFlags flags, std::filesystem::path defaultDirectory)
	: width_(700), height_(450), posX_(0), posY_(0), flags_(flags), defaultDirectory_(std::move(defaultDirectory)), shouldOpen_(false), shouldClose_(false), isOpened_(false), isOk_(false),
	  isPosSet_(false), rangeSelectionStart_(0), editDir_(false), setFocusToEditDir_(false)
{
...
inline void ImGui::FileBrowser::Display()
{
	...
	if (editDir_)
	{
		...
		const bool enter = InputText("##directory", currDirBuffer_.data(), currDirBuffer_.size(),
									 ImGuiInputTextFlags_CallbackResize | ImGuiInputTextFlags_EnterReturnsTrue | ImGuiInputTextFlags_AutoSelectAll, ExpandInputBuffer, &currDirBuffer_);
		PopItemWidth();
	...
	if (SmallButton("*"))
	{
		...
		{
			const auto it = std::find_if(fileRecords_.begin(), fileRecords_.end(), [&](const FileRecord& record) { return name == record.name; });
...

	{
		...
		for (unsigned int rscIndex = 0; rscIndex < fileRecords_.size(); ++rscIndex)
		{
			...
			if (Selectable(rsc.showName.c_str(), selected, selectableFlag))
			{
				...
				const bool rangeSelect = canSelect && GetIO().KeyShift && rangeSelectionStart_ < fileRecords_.size() && (flags_ & ImGuiFileBrowserFlags_MultipleSelection)
	...
	if (flags_ & ImGuiFileBrowserFlags_EnterNewFilename)
	{
		...
		PushItemWidth(-1);
		if (InputText("", inputNameBuffer_.data(), inputNameBuffer_.size(), ImGuiInputTextFlags_CallbackResize, ExpandInputBuffer, &inputNameBuffer_) && inputNameBuffer_[0] != '\0')
		{
			selectedFilenames_ = {u8StrToPath(inputNameBuffer_.data())};
		}
		focusOnInputText |= IsItemFocused();
	...
	const bool shouldClose =
		Button("cancel") || shouldClose_ || ((flags_ & ImGuiFileBrowserFlags_CloseOnEsc) && IsWindowFocused(ImGuiFocusedFlags_RootAndChildWindows) && IsKeyPressed(ImGuiKey_Escape));
	if (shouldClose)
	{
		CloseCurrentPopup();
	}
...
inline void ImGui::FileBrowser::UpdateFileRecords()
{
	...
	if (fileRecords_.size() > 2)
	{
		...
		std::sort(fileRecordRemapIndices.begin() + 1, fileRecordRemapIndices.end(), [&](uint32_t li, uint32_t ri) { return keys[li] < keys[ri]; });
...
inline void ImGui::FileBrowser::SetCurrentDirectoryUncatched(const std::filesystem::path& pwd)
{
	...
	if ((flags_ & ImGuiFileBrowserFlags_EnterNewFilename) && selectedFilenames_.size() == 1 && !customizedInputName_.empty() && !inputNameBuffer_.empty()
...
inline bool ImGui::FileBrowser::SetCurrentDirectoryInternal(const std::filesystem::path& dir, const std::filesystem::path& preferredFallback)
{
	try
	{
